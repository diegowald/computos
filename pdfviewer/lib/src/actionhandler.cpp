...
//				m_goToPageSlot = (char *)slot; // XXX this seems to work, if an unknown crash occurs, replacing this by the above might be a better idea
		...
		case PdfView::ShowForms:
            ...
            break;
    case PdfView::MouseToolRedlining:
        if (!m_mouseToolsActionGroup)
            m_mouseToolsActionGroup = new QActionGroup(this);
        if (!m_mouseRedliningAction)
        {
            m_mouseRedliningAction = new QAction(tr("&Redlining Tool", "Action: choose mouse tool"), this);
            m_mouseRedliningAction->setCheckable(true);
            m_mouseRedliningAction->setData(qVariantFromValue(PdfView::Redlining));
            m_mouseRedliningAction->setObjectName(QLatin1String("pdfview_mouse_redlining"));
            connect(m_mouseRedliningAction, SIGNAL(triggered()), receiver, slot);
            m_mouseToolsActionGroup->addAction(m_mouseRedliningAction);
        }
        return m_mouseRedliningAction;
        break;
