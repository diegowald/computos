...
void PdfViewPrivate::slotRedliningMouseTool()
{
    QAction *action = qobject_cast<QAction*>(sender());
    if (action)
    {
...
QAction *PdfView::action(PdfViewAction action)
{
    ...
    case NextRedline:
        ...
        if (action == Redlines)
            ...
            return d->m_redliningHandler->action(0);
        else if (action == PreviousRedline)
            return d->m_redliningHandler->action(1);
        else if (action == NextRedline)
            return d->m_redliningHandler->action(2);
        break;
    ...
    case MouseToolRedlining:
        return d->m_actionHandler->action(action, d, SLOT(slotRedliningMouseTool()));
        break;
...
void PdfViewPrivate::addRedlineRect(const QPointF &scenePos)
{
    m_redliningStart = scenePos;
    m_redliningRect = m_pageScene->addRect(m_redliningStart.x(),
                                           m_redliningStart.y(),
                                           0, 0,
                                           QPen(QBrush(QColor(100, 0, 0, 100)), 1),
                                           QBrush(QColor(100, 0, 0, 100)));
    m_redliningRect->setZValue(2);
}

void PdfViewPrivate::removeRedlineRect()
{
    if (m_redliningRect)
    {
        m_pageScene->removeItem(m_redliningRect);
        delete m_redliningRect;
        m_redliningRect = 0;
    }
}
...
void PdfViewPrivate::handleRedlining(const QPoint &popupMenuPos)
{
    QRectF redlineRect = m_redliningRect->rect();
    if (redlineRect.width() < 1 || redlineRect.height() < 1)
    {
        removeRedlineRect();
        return;
    }
...
m_Redline.rect =redlineRect;
    ...
    QMenu menu(q);
// ACA DEBO DEFINIR
    QAction *createRedlineAction = menu.addAction(tr("Create new redLine"));
...
//		QApplication::setOverrideCursor(Qt::SizeAllCursor);
    ...
    else if (d->m_mouseTool == Redlining)
    {
        d->m_isDragging = true;
#ifndef QT_NO_CURSOR
        QApplication::setOverrideCursor(Qt::CrossCursor);
#endif
        d->addRedlineRect(mapToScene(event->pos()));
    }
}
...
void PdfView::mouseMoveEvent(QMouseEvent *event)
{
    if (d->m_isDragging)
    {
        ...
        {
            const QPointF pos = mapToScene(event->pos());
            const double redlineRectLeft = qMin(pos.x(), d->m_redliningStart.x());
            const double redlineRectTop = qMin(pos.y(), d->m_redliningStart.y());
            const double redlineRectWidth = qAbs(pos.x() - d->m_redliningStart.x());
            const double redlineRectHeight = qAbs(pos.y() - d->m_redliningStart.y());
            d->m_redliningRect->setRect(redlineRectLeft, redlineRectTop, redlineRectWidth, redlineRectHeight);
        }
        return;
    }
...
void PdfView::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    else if (d->m_mouseTool == Redlining && d->m_isDragging)
    {
        d->m_isDragging = false;
#ifndef QT_NO_CURSOR
        QApplication::restoreOverrideCursor();
#endif
        d->handleRedlining(event->globalPos());
        d->removeRedlineRect();
    }
}
